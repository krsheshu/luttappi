...
from sim_reduction_streaming_chain_mult import sim_streaming_chain_mult, check_simulation_results
#from streaming_chain_mult_convert import streaming_chain_mult_convert
...
TB_NAME = 'sim_streaming_chain_mult'           # test file
SYNTHESIS_TOP = 'syn_streaming_chain_mult_top' # synthesis top level
...
def simulate(sim_temp_dir, sim_pattern_dir, valid_pattern, ready_pattern, nb_frames,sim_time=None):
  ...
  tb=traceSignals(sim_streaming_chain_mult,pars_obj)
  sim=Simulation(tb)
  if (sim_time is None):
    sim.run()
  else:
