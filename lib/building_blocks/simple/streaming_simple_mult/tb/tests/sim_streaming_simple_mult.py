...
def sim_streaming_simple_mult(pars_obj):
  PATTERN_WIDTH = 16
  INP_DATA_WIDTH = 8
  ...
  mult_pars=StreamingSimpleMultPars()
  mult_pars.SNK0_DATA_WIDTH=INP_DATA_WIDTH
  mult_pars.SNK1_DATA_WIDTH=INP_DATA_WIDTH
  mult_pars.SRC_DATA_WIDTH=OP_DATA_WIDTH
  mult_pars(mult_pars)
  mult_i=StreamingSimpleMult()
  ...
  streaming_simple_mult_inst = mult_i.block_connect(mult_pars, reset, clk, av_snk0, av_snk1, av_src)
...
def check_simulation_results(pars_obj):
  ...
  print("Operation intended: Multiplication")
  ...
  if (len(recv_data) < MAX_NB_TRANSFERS):
    ...
    if (err_cnt):
      ...
      print("Receive and transmit data exactly matches for multiplication...") 
