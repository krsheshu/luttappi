...
from sim_streaming_simple_mult import sim_streaming_simple_mult, check_simulation_results
#from streaming_simple_mult_convert import streaming_simple_mult_convert

#------script parameters------#
DEF_VALID_READY=0xffff
DEFAULT_SIM_TIME=2000
TB_NAME = 'sim_streaming_simple_mult'           # test file
SYNTHESIS_TOP = 'syn_streaming_simple_mult_top' # synthesis top level
...
def simulate(sim_temp_dir, sim_pattern_dir, valid_pattern, ready_pattern, nb_frames,sim_time=None):
  ...
  tb=traceSignals(sim_streaming_simple_mult,pars_obj)
  sim=Simulation(tb)
  if (sim_time is None):
    sim.run()
  else:
