...
def displayAccOut(nbR,accu_in,i):
    ...
    if(accu_in.valid == 1):
      print("{}: nb:{}: accu_out{}: {} ".format(myhdl.now(), nbR, i,accu_in.data))
...
def sim_logistic_regression_2(pars_obj):
  ...
  moduleLR0 = LogisticRegression ( NB_PIPELINE_STAGES, DATAWIDTH, CHANNEL_WIDTH, INIT_DATA, LEN_THETA, CMD_FILE  )
  moduleLR1 = LogisticRegression ( NB_PIPELINE_STAGES, DATAWIDTH, CHANNEL_WIDTH, INIT_DATA, LEN_THETA, CMD_FILE  )
  moduleLR2 = LogisticRegression ( NB_PIPELINE_STAGES, DATAWIDTH, CHANNEL_WIDTH, INIT_DATA, LEN_THETA, CMD_FILE  )
  moduleLR3 = LogisticRegression ( NB_PIPELINE_STAGES, DATAWIDTH, CHANNEL_WIDTH, INIT_DATA, LEN_THETA, CMD_FILE  )
  moduleLR4 = LogisticRegression ( NB_PIPELINE_STAGES, DATAWIDTH, CHANNEL_WIDTH, INIT_DATA, LEN_THETA, CMD_FILE  )
  moduleLR5 = LogisticRegression ( NB_PIPELINE_STAGES, DATAWIDTH, CHANNEL_WIDTH, INIT_DATA, LEN_THETA, CMD_FILE  )
  moduleLR6 = LogisticRegression ( NB_PIPELINE_STAGES, DATAWIDTH, CHANNEL_WIDTH, INIT_DATA, LEN_THETA, CMD_FILE  )
  moduleLR7 = LogisticRegression ( NB_PIPELINE_STAGES, DATAWIDTH, CHANNEL_WIDTH, INIT_DATA, LEN_THETA, CMD_FILE  )
  moduleLR8 = LogisticRegression ( NB_PIPELINE_STAGES, DATAWIDTH, CHANNEL_WIDTH, INIT_DATA, LEN_THETA, CMD_FILE  )
  ...
  def receive_data_process():
    ...
    if (moduleLR9.pipe_out_acc.valid == 1):
      ...
      if (nbR == NB_TRAINING_SAMPLES):
        ...
        raise myhdl.StopSimulation("Simulation Finished in %d clks: In total " %myhdl.now() + str(MAX_NB_TRANSFERS) + " data words transfered")
  ...
  def simulation_time_check():
    ...
    if(sim_time_now>MAX_SIM_TIME):
        raise myhdl.StopSimulation("Warning! Simulation Exited upon reaching max simulation time of " + str(MAX_SIM_TIME) + " clocks")
