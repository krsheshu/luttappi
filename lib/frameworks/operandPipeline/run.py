...
from sim_operand_pipeline import sim_operand_pipeline, check_simulation_results
#from operand_pipeline_convert import operand_pipeline_convert

#------script parameters------#
DEF_VALID_READY=0xffff
DEFAULT_SIM_TIME=2000
TB_NAME = 'sim_operand_pipeline'           # test file
SYNTHESIS_TOP = 'syn_operand_pipeline_top' # synthesis top level
...
def simulate(sim_temp_dir, sim_pattern_dir, valid_pattern, ready_pattern, nb_frames,sim_time=None):
  ...
  tb=traceSignals(sim_operand_pipeline,pars_obj)
  sim=Simulation(tb)
  if (sim_time is None):
    sim.run()
  else:
